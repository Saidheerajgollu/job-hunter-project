...
- **Using `validThrough` (schema.org's own listing-expiration field) as a staleness signal.** Worth reconsidering later, but not on the grounds originally stated here — correction made after the final review found the original justification factually wrong. Schema-org jobs are NOT covered by sub-project 2's closer sweep: `closeStaleJobs` is only ever called from `fastPoll.js`, scoped to its hardcoded list of 7 direct-ATS sources (which correctly excludes `schema-org`, since this sub-project deliberately doesn't touch the main feed — see the Non-goal above), and `companyWatcher.js`'s own watchlist diff logic never calls `closeStaleJobs` for any source. The real, load-bearing consequence: schema-org jobs, once inserted, are never marked closed by any existing mechanism and will accumulate indefinitely as permanently "open" in the feed. This is not a regression (the `handleCustomExtract`/`handleCustomHash` tier this replaces never closed jobs either), but it's a real gap worth a deliberate decision in a future pass — either building watchlist-side staleness detection, or using `validThrough` as a lightweight signal specific to this source, rather than continuing to accumulate silently.
