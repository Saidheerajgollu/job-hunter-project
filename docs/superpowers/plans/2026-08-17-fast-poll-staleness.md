...
**Ruling applied during Task 4 (see progress.md ledger):** `fetchWorkdayJobs` already returns `[]` (not a throw) when every search term's request fails — it `break`s out of the term loop on a non-`ok` response or a caught error, leaving `allJobs` at whatever it had accumulated (`[]` if the very first term failed) — and it cannot distinguish "the endpoint is genuinely down" from "the endpoint answered fine and this company just has zero matching postings right now." Because of that ambiguity, `polledCompanies.push(company)` is conditional on `postings.length > 0`, not unconditional: a company only counts as "polled" when at least one job was actually found. This is a deliberate departure from this plan's general rule that zero postings still counts as a successful poll — for Workday specifically, treating "zero postings" as "not polled" is the safer failure mode, since the alternative risks a transient endpoint outage being silently read as "confirmed zero jobs," which would eventually and incorrectly close real open listings. The accepted cost: a Workday company that goes from having open jobs to genuinely zero open jobs won't have its stale listings auto-closed by this mechanism until a future poll finds at least one job there again.
