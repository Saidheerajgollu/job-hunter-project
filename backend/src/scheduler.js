 ...
 *   1. Main scraper   — Greenhouse / Lever / Ashby / Workday / SimplifyJobs / etc.
 *                       Runs immediately on boot, then every hour (cheap API calls).
 *   2. Company watcher — diffs each watched company's ATS board and fires push alerts.
 *                       Runs every 10 minutes so watched companies are caught fast.
 *   3. JSearch        — LinkedIn / Indeed / Glassdoor aggregation via RapidAPI.
 *                       Only runs if RAPIDAPI_KEY is set. Every 12 hours to respect
 *                       the free tier (~240 calls/month).
 *   4. Fantastic.jobs — broad ATS-board feed (Active Jobs DB) via RapidAPI.
...
export function startScheduler() {
    console.log('⏰ Scheduler initialized — main scraper hourly, watcher every 10 min, JSearch every 12h');

    // Main scraper: run immediately on startup, then every hour
    runScraperSafe();
    cron.schedule('0 * * * *', () => runScraperSafe());

    // Company watcher: first run 1 min after startup, then every 10 minutes.
    // The isWatcherRunning guard skips a cycle if the previous one is still going.
    setTimeout(() => runWatcherSafe(), 60 * 1000);
    cron.schedule('*/10 * * * *', () => runWatcherSafe());
